...
class SimOTAAssigner(BaseAssigner):
    ...
    def _assign(self,
        ...
        with torch.cuda.amp.autocast(enabled=False):
            cls_cost = F.binary_cross_entropy(valid_pred_scores.to(dtype=soft_label.dtype), soft_label, reduction="none") * scale_factor.abs().pow(2.0)
        # cls_cost = F.binary_cross_entropy(valid_pred_scores.to(dtype=soft_label.dtype), soft_label, reduction="none") * scale_factor.abs().pow(2.0)

        cls_cost = cls_cost.sum(dim=-1)
        #print('cls', cls_cost.shape, 'iou', iou_cost.shape)
        cost_matrix = (
            cls_cost * self.cls_weight + iou_cost * self.iou_weight +
            (~is_in_boxes_and_center) * INF)
        #print('cls', cls_cost.min(), cls_cost.max(), cls_cost.mean(),cls_cost.shape)
        #print('iou', iou_cost.min(), iou_cost.max(), iou_cost.mean(), iou_cost.shape, )
